...
var (
	...
	maxUploadTuples      = flag.Int("max-upload-tuples", 100000, "Maximum number of tuples per single Castor upload. Jobs producing more tuples are split into multiple chunks with deterministically derived IDs.")
)
...
func main() {
	...
	if err = controllers.NewTupleGenerationJobReconciler(
		...
		castorClient,
		mgr.GetLogger(),
		*maxUploadTuples).SetupWithManager(mgr); err != nil {

...
reset:
    ...
    // Use compile-time constant KEY_LENGTH for static analysis
    ...
    // Explicit validation: ensure copy_len is within safe bounds before memcpy
    if (copy_len <= KEY_LENGTH && copy_len > 0)
    {
        // Safe to copy - destination buffer is at least KEY_LENGTH bytes
        memcpy(Seed, hex_result, copy_len);
    }
    else
    {
        fprintf(stderr, "Error: Invalid copy size for Seed memcpy\n");
        free(hex_result);
        goto exit;
    }
    ...
    if (Player_MAC_Keys_p[other_player_number] != NULL)
    {
        ...
        memcpy(Player_MAC_Keys_p[other_player_number], message->mackeyshare_p, MAC_KEY_BUF_SIZE);
    }
    else
    {
    ...
    {
        // Safe to copy: destination buffer is MAC_KEY_BUF_SIZE bytes, copying MAC_KEY_BUF_SIZE bytes
        memcpy(Player_MAC_Keys_2[other_player_number], message->mackeyshare_2, MAC_KEY_BUF_SIZE);
    }
    else
    {
